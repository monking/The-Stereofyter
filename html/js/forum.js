	...
	$.fn.forum = function(options) {
		...
		var draw = {
			detail: function() {
                ...
                var post = thread[options.offset];
                ...
                $("form.reply", $detailContainer).unbind("submit").submit(function(event) {
                    event.preventDefault();
                    // $.ajax();
                });
			},
			list: function() {
				...
				paging.pages = Math.ceil(thread.length / paging.limit);
				for (var i = paging.page * paging.limit; i < paging.offset + paging.limit && i < thread.length; i++) {
                    var title = thread[i].link_name || thread[i].title || 'New Post';
					var markup = '<li class="entry" ref="'+thread[i].id+'">';
					markup += '<h4 class="title">'+title+'</h4>';
					markup += '<div class="message">'+thread[i].message+'<div class="tail-overlay"></div></div>';
					markup += '</li>';
					$list.append(markup);
				}
				...
				$("li.entry", $list).click(function(event) {
					event.preventDefault();
					show("detail", threadXRef[parseInt($(this).attr("ref"))]);
				});
			}
		};
