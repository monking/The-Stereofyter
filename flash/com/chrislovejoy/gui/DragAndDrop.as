package com.chrislovejoy.gui {
	...
	public class DragAndDrop extends Sprite {
		...
		protected function startMyDrag(event:Event = null):void {
			...
			if (!options.isGhost) {
				...
				startSnapGhost(); /* after dispatchEvent in case a listener adds this instance to another parent */
			}
		}
	...
	protected function mouseUpListener(event:Event):void {
		if (mouseUpBuffer <= 0) stopMyDrag(event);
		mouseUpBuffer > 0 && (mouseUpBuffer--);
	}
